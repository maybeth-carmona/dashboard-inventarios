...
df["dias_demora"] = (HOY - pd.to_datetime(df["fecha_entrega"])).dt.days.fillna(0).astype(int)
...
# KPIs (CUADROS)
# ======================================
kpi_riesgo = df_view[df_view["dias_demora"] > 30]["pedido"].nunique()
kpi_atraso = df_view[df_view["dias_demora"] > 0]["pedido"].nunique()

c1, c2 = st.columns(2)
c1.metric("Pedidos en riesgo", kpi_riesgo)
